// PWA client for Becklan V2 API (prefilled + Service Performed)
...
// Service (includes Service Performed -> service_type)
document.getElementById('btnService').addEventListener('click', async () => {
  try{ const payload = { path:'record/service',
      patient: document.getElementById('svcPatient').value,
      fee: Number(document.getElementById('svcFee').value||0),
      diagnosis: document.getElementById('svcDiagnosis').value,
      staff: document.getElementById('svcStaff').value,
      service_type: document.getElementById('svcType').value,
      medication: document.getElementById('svcMedication').value };
    const data = await postJSON(base(), payload);
    if(data.ok) ok('Service recorded.'); else err('Service failed.');
  }catch(e){ err('Service failed: ' + e.message); }
});
